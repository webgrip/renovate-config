...
function handleCommitError(err, branchName, files) {
	...
	if (err.message.includes("refusing to allow a GitHub App to create or update workflow")) {
		logger.warn("App has not been granted permissions to update Workflows - aborting branch.");
		return null;
	}
	if ((err.message.includes("remote rejected") || err.message.includes("403")) && files?.some((file) => file.path?.startsWith(".github/workflows/"))) {
	...
	if (err.message.includes("denying non-fast-forward") || err.message.includes("GH003: Sorry, force-pushing")) {
...
//#endregion
export { bulkChangesDisallowed, checkForPlatformFailure, handleCommitError };
