...
var ForgejoTagsDatasource = class ForgejoTagsDatasource extends Datasource {
	...
	static getApiUrl(registryUrl) {
		return `${ensureTrailingSlash(ForgejoTagsDatasource.getRegistryURL(registryUrl).replace(regEx(/\/api\/v1$/), ""))}api/v1/`;
	}
	...
	static getSourceUrl(packageName, registryUrl) {
		return `${ensureTrailingSlash(ForgejoTagsDatasource.getRegistryURL(registryUrl))}${packageName}`;
	}
	...
	getReleases(config) {
		return withCache({
			...
			key: ForgejoTagsDatasource.getCacheKey(config.registryUrl, config.packageName, "tags"),
			fallback: true
