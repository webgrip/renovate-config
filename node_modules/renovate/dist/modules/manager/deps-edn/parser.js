...
import moo from "moo";
//#region lib/modules/manager/deps-edn/parser.ts
const lexerStates = {
	main: {
		comma: { match: "," },
		lineComment: { match: /;.*?$/ },
		...
		symbol: { match: /(?:[a-zA-Z*+!_'?<>=.-][a-zA-Z0-9*+!_'?<>=.-]*)(?:\/(?:[a-zA-Z*+!_'?<>=.-][a-zA-Z0-9*+!_'?<>=.-]*))?/ },
		double: { match: /(?:[0-9]+\.[0-9]*|[0-9]*\.[0-9]+)(?:[eE][+-]?[0-9]+)?|(?:[0-9]+[eE][+-]?[0-9]+)/ },
...
function parseDepsEdnFile(content) {
	...
	const popState = () => {
		...
		if (savedState.type === "record") if (savedState.skipKey) {
			...
			savedState.currentKey = null;
		} else savedState.skipKey = true;
		if (savedState.type === "array") savedState.data.push(state.data);
		state = savedState;
		return true;
	};
	for (const token of tokens) {
		...
		if (tokenType === "lineComment" || tokenType === "unknown" || tokenType === "doubleQuoted" || tokenType === "longDoubleQuoted" || tokenType === "stringFinish" || tokenType === "comma") continue;
		else if (tokenType === "rightParen" || tokenType === "rightSquare" || tokenType === "rightFigure") {
		...
		} else if (tokenType === "symbol" || tokenType === "keyword" || tokenType === "stringContent" || tokenType === "double" || tokenType === "rational" || tokenType === "integer") {
			if (stateType === "record") if (state.skipKey) {
				state.currentKey = null;
				state.skipKey = false;
			} else if (state.currentKey) {
				state.data[state.currentKey] = token.value;
				state.currentKey = null;
			} else state.currentKey = token.value;
			else if (stateType === "array") state.data.push(token.value);
			else if (stateType === "root") state.data = token.value;
		} else {
			const unknownType = tokenType;
			logger.debug({ unknownType }, `Unknown token type for "deps.edn"`);
		}
	}
	while (stack.length) popState();
	if (isPlainObject(state.data)) return {
		data: state.data,
		metadata
	};
	return null;
