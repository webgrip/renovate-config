...
import { ConfigMigrationCommitMessageFactory } from "../branch/commit-message.js";
...
async function ensureConfigMigrationPr(config, migratedConfigData) {
	...
	const docsLink = joinUrlParts(coerceString(config.productLinks?.documentation), "configuration-options/#configmigration");
...
${filename.endsWith(".json5") ? `#### [PLEASE NOTE](${docsLink}): JSON5 config file migrated! All comments & trailing commas were removed.` : ""}
...
:question: Got questions? Does something look wrong to you? Please don't hesitate to [request help here](${config.productLinks?.help}).\n\n`);
	...
	if (existingPr) {
		...
		if (existingPr.bodyStruct?.hash === prBodyHash && existingPr.title === prTitle) {
			logger.debug(`Pr does not need updating, PrNo: ${existingPr.number}`);
			return existingPr;
		}
		if (GlobalConfig.get("dryRun")) logger.info("DRY-RUN: Would update migration PR");
		else {
	...
	try {
		if (GlobalConfig.get("dryRun")) logger.info("DRY-RUN: Would create migration PR");
		else {
		...
		if (err.response?.statusCode === 422 && err.response?.body?.errors?.[0]?.message?.startsWith("A pull request already exists")) {
			logger.warn({ err }, "Migration PR already exists but cannot find it. It was probably created by a different user.");
...
//#endregion
export { ensureConfigMigrationPr };
