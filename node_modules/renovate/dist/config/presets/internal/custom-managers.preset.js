//#region lib/config/presets/internal/custom-managers.preset.ts
const presets = {
	azurePipelinesVersions: {
		customManagers: [{
			customType: "regex",
			managerFilePatterns: ["**/.azuredevops/**/*.{yml,yaml}", "azure*pipeline*.{yml,yaml}"],
			matchStrings: ["# renovate: datasource=(?<datasource>[a-zA-Z0-9-._]+?) depName=(?<depName>[^\\s]+?)(?: (?:lookupName|packageName)=(?<packageName>[^\\s]+?))?(?: versioning=(?<versioning>[^\\s]+?))?(?: extractVersion=(?<extractVersion>[^\\s]+?))?(?: registryUrl=(?<registryUrl>[^\\s]+?))?\\s+[A-Za-z0-9_]+?_VERSION\\s*:\\s*[\"']?(?<currentValue>.+?)[\"']?\\s"]
		}],
	...
	gitlabPipelineVersions: {
		customManagers: [{
			...
			matchStrings: ["# renovate: datasource=(?<datasource>[a-zA-Z0-9-._]+?) depName=(?<depName>[^\\s]+?)(?: (?:packageName)=(?<packageName>[^\\s]+?))?(?: versioning=(?<versioning>[^\\s]+?))?(?: extractVersion=(?<extractVersion>[^\\s]+?))?(?: registryUrl=(?<registryUrl>[^\\s]+?))?\\s+[A-Za-z0-9_]+?_VERSION\\s*:\\s*[\"']?(?<currentValue>.+?)[\"']?\\s"]
		}],
	...
	mavenPropertyVersions: {
		customManagers: [{
			...
			matchStrings: ["<!--\\s?renovate:( datasource=(?<datasource>[a-zA-Z0-9-._]+?))? depName=(?<depName>[^\\s]+?)(?: packageName=(?<packageName>[^\\s]+?))?(?: versioning=(?<versioning>[^\\s]+?))?(?: extractVersion=(?<extractVersion>[^\\s]+?))?\\s+-->\\s+<.+\\.version>(?<currentValue>.+)<\\/.+\\.version>"],
	...
	tfvarsVersions: {
		customManagers: [{
			...
			matchStrings: ["#\\s*renovate: datasource=(?<datasource>.*?) depName=(?<depName>.*?)( versioning=(?<versioning>.*?))?(?: extractVersion=(?<extractVersion>.*?))?(?: registryUrl=(?<registryUrl>[^\\s]+?))?\\s.*?_version\\s*=\\s*\"(?<currentValue>.*)\""],
			versioningTemplate: "{{#if versioning}}{{{versioning}}}{{/if}}"
		}],
...
//# sourceMappingURL=custom-managers.preset.js.map