// Release config for the Webgrip shared Renovate preset.
//
// Forge-agnostic: ONE config, correct on both Forgejo (leading) and GitHub (mirror). The publish
// plugin is gated on GITEA_ACTIONS — an intrinsic Forgejo-runner env var (=true on Forgejo, unset on
// GitHub; GITHUB_ACTIONS is set on BOTH so it can't discriminate). No org var / no vars.* lookup.
...
const releaseAssets = ['default.json', 'grouped.json', 'safe-automerge.json', 'gitops.json'];
...
const exec = [
    ...
    { verifyReleaseCmd: "npm run validate && echo '✅ All presets passed validation'" },
];
...
const giteaPublish = ['@saithodev/semantic-release-gitea', { assets: releaseAssets.map((path) => ({ path })) }];
...
const githubPublish = [
    ...
    {
        assets: [
            { path: 'default.json', name: 'default.json', label: 'Default preset (v${nextRelease.version})' },
            { path: 'grouped.json', name: 'grouped.json', label: 'Grouped overlay (v${nextRelease.version})' },
            { path: 'safe-automerge.json', name: 'safe-automerge.json', label: 'Safe-automerge overlay (v${nextRelease.version})' },
            { path: 'gitops.json', name: 'gitops.json', label: 'GitOps overlay (v${nextRelease.version})' },
        ],
        successComment:
            '🎉 This change ships in [v${nextRelease.version}](${nextRelease.url}).\n\n' +
            'Update consuming repos to adopt it:\n\n' +
            '```json\n{\n  "extends": ["github>webgrip/renovate-config#v${nextRelease.version}"]\n}\n```',
        failComment:
            'The release workflow failed to publish a new version. Check the [Actions tab](https://github.com/webgrip/renovate-config/actions) for details.',
...
module.exports = {
    branches: ['main', { name: 'next', channel: 'rc', prerelease: 'rc' }],
    tagFormat: 'v${version}',
